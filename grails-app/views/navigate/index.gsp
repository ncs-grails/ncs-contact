            ...
            <div id="content">
                ...
                <g:if test="${organizationInstance}">
	                <div id="contentOrganization" class="roundrect institution entity">
	                    <h1>${fieldValue(bean:organizationInstance, field:'name')}
                            <g:link class="editentity" controller="organizationalUnit" action="edit" id="${OrganizationalUnit.findByUmbrellaAndOrganization(true, organizationInstance).id}">edit</g:link>
	                    </h1>
                        <g:if test="${fieldValue(bean:organizationInstance, field:'nickName')}">
                ...
                <g:if test="${personInstanceList}">
                    <div id="contentSearch" class="roundrect search entity">
                        <h1>Search Results</h1>
	                        <g:each in="${personInstanceList}" status="i" var="personInstance">
						        <div class="roundrect person entity">
						            <h1>
	                                    <g:link action="person" id="${personInstance.id}">
                ...
                <g:if test="${personInstance}">
                    <div id="contentPerson" class="roundrect person entity" style="width: 93%;">
                        <h1>
                            ${personInstance.getPreferredName()?.toString()?.encodeAsHTML()}
						    ...
						    <g:if test="${personInstance.image}">
							    <g:if test="${personInstance.image}">
    							    <div class="roundrect photomatte">
    								    <img src="${createLink(controller:'image', action:'show', id: personInstance?.image?.id)}" />
								    </div
							    </g:if>
						    </g:if>
                        ...
                        <g:each in="${personInstance.addresses.sort{it.preferredOrder}}" status="i" var="a">
                        ...
                        <g:if test="${personInstance.addresses}"><br/></g:if>


                        <g:each in="${personInstance.phones.sort{it.preferredOrder}}" status="i" var="p">
                            <div class="roundrect phone entity">
    						<h2>${p?.phoneType?.name}</h2>
    						<p>${p?.phone?.toString()}</p>
    						</div>
                        </g:each>                        
                        <g:if test="${personInstance.phones}"><br/></g:if>


                        <g:each in="${personInstance.uris.sort{it.preferredOrder}}" status="i" var="u">
                        ...
                        <g:if test="${personInstance.uris}"><br/></g:if>

                        <div id="contentFooter">
                            <div class="roundrect" id="groupMemberships" style="background-color: #fff;">
                                <h1>Groups:</h1>
                            ...
                            <div class="roundrect" id="orgMemberships" style="background-color: #fff;">
                                <h1>Organizations:</h1>
